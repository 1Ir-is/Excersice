...
public class GPTServiceImpl implements GPTService {
    ...
    public CompletableFuture<String> generateTopicsFromCampaign(Campaign campaign, Integer numberOfTopics, String additionalInstructions) {
        try {
            log.info("🇻🇳 Generating {} Vietnamese topics for campaign '{}' for", numberOfTopics, campaign.getName());
            ...
            requestDTO.setMessages(Arrays.asList(new GPTMessage("system", "Bạn là một chuyên gia marketing người Việt Nam với 10 năm kinh nghiệm. " + "Bạn hiểu rõ thị trường Việt Nam, văn hóa, ngôn ngữ và hành vi tiêu dùng. " + "Hãy tạo các chủ đề marketing bằng tiếng Việt thuần túy, phù hợp với người Việt. " + "QUAN TRỌNG: Chỉ trả lời bằng tiếng Việt, không dùng tiếng Anh. " + "User: 1Ir-is - Time: 2025-08-07 09:32:01"), new GPTMessage("user", prompt)));

            GPTResponseDTO responseDTO = callGPTAPI(requestDTO).get();
            String content = responseDTO.getChoices().get(0).getMessage().getContent();

            log.info("Successfully generated Vietnamese topics for");
            log.debug("Generated content: {}", content);

            return CompletableFuture.completedFuture(content);

        } catch (Exception e) {
            log.error("Error generating Vietnamese topics for: {}", e.getMessage(), e);
    ...
    public CompletableFuture<String> generateContentFromTopic(Topic topic, String tone, String contentType, String additionalInstructions) {
        try {
            log.info("🇻🇳 Generating Vietnamese {} content for topic '{}' for", contentType, topic.getName());

            String prompt = buildVietnameseContentGenerationPrompt(topic, tone, contentType, additionalInstructions);

            GPTRequestDTO requestDTO = new GPTRequestDTO();
            requestDTO.setModel("gpt-3.5-turbo");
            requestDTO.setMax_tokens(1200); // Increased for better content
            requestDTO.setTemperature(0.8);
            requestDTO.setMessages(Arrays.asList(new GPTMessage("system", "Bạn là một copywriter chuyên nghiệp người Việt Nam, chuyên tạo nội dung marketing tiếng Việt. " + "Bạn viết theo phong cách người Việt, sử dụng từ ngữ thân thiện, dễ hiểu. " + "QUAN TRỌNG: Chỉ viết bằng tiếng Việt, không dùng tiếng Anh trừ khi cần thiết cho hashtag. " + "User: 1Ir-is - Time: 2025-08-07 09:32:01"), new GPTMessage("user", prompt)));

            GPTResponseDTO responseDTO = callGPTAPI(requestDTO).get();
            String content = responseDTO.getChoices().get(0).getMessage().getContent();

            log.info("Successfully generated Vietnamese content for");
            return CompletableFuture.completedFuture(content);

        } catch (Exception e) {
            log.error("Error generating Vietnamese content for: {}", e.getMessage(), e);
    ...
    public CompletableFuture<String> generateLongFormContent(Topic topic, ContentGenerationRequestDTO request) {
        try {
            log.info("🇻🇳 Generating long-form {} content ({} words) for topic '{}' for at 2025-08-07 09:32:01", request.getContentType(), request.getTargetWordCount(), topic.getName());
            ...
            requestDTO.setTemperature(0.7);
            requestDTO.setMessages(Arrays.asList(new GPTMessage("system", buildLongFormSystemPrompt(request)), new GPTMessage("user", prompt)));

            GPTResponseDTO responseDTO = callGPTAPI(requestDTO).get();
            String content = responseDTO.getChoices().get(0).getMessage().getContent();

            log.info("Successfully generated long-form Vietnamese content ({} words estimated) for", request.getTargetWordCount());

            return CompletableFuture.completedFuture(content);

        } catch (Exception e) {
            log.error("Error generating long-form Vietnamese content for: {}", e.getMessage(), e);
            throw new RuntimeException("Không thể tạo nội dung dài tiếng Việt", e);
        }
    }
    ...
    public CompletableFuture<String> generateImagePromptFromContent(String content) {
        try {
            log.info("🇻🇳 Generating image prompt for Vietnamese content for");
            ...
            requestDTO.setMessages(Arrays.asList(new GPTMessage("system", "Bạn là chuyên gia tạo prompt cho AI image generation. " + "Hãy tạo prompt tiếng Anh ngắn gọn, chính xác cho DALL-E hoặc Midjourney. " + "User: 1Ir-is - Time: 2025-08-07 09:32:01"), new GPTMessage("user", prompt)));

            GPTResponseDTO responseDTO = callGPTAPI(requestDTO).get();
            String imagePrompt = responseDTO.getChoices().get(0).getMessage().getContent();

            log.info("Successfully generated image prompt for");
            return CompletableFuture.completedFuture(imagePrompt);

        } catch (Exception e) {
            log.error("Error generating image prompt for: {}", e.getMessage(), e);
    ...
    public CompletableFuture<GPTResponseDTO> callGPTAPI(GPTRequestDTO request) {
        try {
            ...
            log.error("OpenAI API call failed for: {}", e.getMessage(), e);
